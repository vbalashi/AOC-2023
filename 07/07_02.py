...
# redefining only J compared to first task
...
def order_of_strength(hand: str):
    # use first iteration to identify which is the best for hand type
    kind_dictionary = {}
    for kind in hand:
        kind_dictionary[kind] = kind_dictionary.get(kind, 0) + 1
    rank = int("".join(map(str, sorted(list(kind_dictionary.values()), reverse=True))))
    # use J to pretend the best other 
    hand_updated = hand.replace('J', max(kind_dictionary, key=lambda k: kind_dictionary[k]))
    kind_dictionary = {}
    for kind in hand_updated:
        kind_dictionary[kind] = kind_dictionary.get(kind, 0) + 1    
    rank_updated = int("".join(map(str, sorted(list(kind_dictionary.values()), reverse=True))))
    print(hand, rank_dictionary.get(rank), hand_updated, rank_dictionary.get(rank_updated))
    return rank_dictionary.get(rank_updated)
